...
class UnetAdaptiveBins(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        return nn.functional.interpolate(pred, scale_factor=2, mode="bilinear")
    ...
    def build(cls, n_bins, **kwargs):
        ...
        print("Changing input channels")
        basemodel.conv_stem = Conv2dSame(4, 48, kernel_size=(3, 3), stride=(2, 2), bias=False)
...
if __name__ == '__main__':
    model = UnetAdaptiveBins.build(25).to("cuda:0")
    x = torch.rand(8, 4, 512, 512).to("cuda:0")
    pred = model(x)
    print(pred[0][0].shape)

    plt.imshow(pred[0][0].detach().cpu().numpy(), cmap="magma")
    plt.colorbar()
    plt.show()
