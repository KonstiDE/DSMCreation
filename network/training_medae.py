...
def train(epoch, loader, loss_fn, optimizer, scaler, model, mse):
    ...
    for batch_index, (data, target, dataframepath) in enumerate(loop):
        ...
        running_medae.append(torch.median(torch.abs(data - target)).item())

        mse.reset()

        loop.set_postfix(info="Epoch {}, train, medae={:.5f}".format(epoch, running_medae[-1]))
        running_loss.append(loss_value)

    return s.mean(running_loss), s.mean(running_mae), \
           s.mean(running_mse), s.mean(running_ssim), \
           s.mean(running_zncc), s.mean(running_medae)
...
def valid(epoch, loader, loss_fn, model, mse):
    ...
    for batch_index, (data, target, dataframepath) in enumerate(loop):
        ...
        loop.set_postfix(info="Epoch {}, valid, medae={:.5f}".format(epoch, running_medae[-1]))
        running_loss.append(loss_value)
...
def run(num_epochs, lr, epoch_to_start_from):
    ...
    overall_training_medae = []
    overall_validation_medae = []
    ...
    for epoch in range(epochs_done + 1, num_epochs + 1):
        training_loss, training_mae, training_mse, training_ssim, training_zncc, training_medae = train(
            epoch,
            train_loader,
            loss_fn,
            optimizer,
        ...
        validation_loss, validation_mae, validation_mse, validation_ssim, validation_zncc, validation_medae = valid(
            epoch,
            validation_loader,
            loss_fn, model,
            torch_mse
        ...
        overall_training_medae.append(training_medae)
        overall_validation_medae.append(validation_medae)
        ...
        torch.save({
            ...
            'training_medae': overall_training_medae,
            ...
            'validation_medaes': overall_validation_medae,
            'early_stopping': early_stopping
        }, path + "model_epoch" + str(epoch) + "_medae.pt")
        ...
        savetxt(path + "metrics_medae.csv", metrics, delimiter=',',
                header="tloss,vloss,tmae,tmse,tssim,tzncc,tmedae,vmae,vmse,vssim,vzncc,vmedae", fmt='%s')
...
if __name__ == '__main__':
    run(num_epochs=18, lr=5e-06, epoch_to_start_from=17)
